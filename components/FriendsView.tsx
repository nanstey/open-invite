...
import { USERS, GROUPS } from '../constants';
import { FriendsMode } from '../types';
...
export const FriendsView: React.FC<FriendsViewProps> = ({ activeTab }) => {
  ...
  const friends = USERS.filter(u => !u.isCurrentUser).filter(u => 
    u.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // Mock group members for display
  const getGroupMembers = (groupId: string) => {
    // Deterministic pseudo-random members based on group ID length
    const count = (groupId.length % 3) + 2; 
    return USERS.slice(1, count + 1);
  };
