...
export interface TouchDragProject {
  id: string;
...
export function ProjectsKanbanBoard({ initialProjectId }: ProjectsKanbanBoardProps) {
  ...
  const [touchDragProject, setTouchDragProject] = useState<{
    id: string;
    sourceStatus: ProjectStatus;
  } | null>(null);
  ...
  const handleMoveProject = async (
    ...
    setProjects(prev =>
      prev.map(p =>
        p.id === projectId ? { ...p, status: targetStatus, sortOrder: newSortOrder } : p
      )
    );
  ...
  // Touch-based drag handlers for mobile support
  const handleTouchDragStart = (projectId: string, sourceStatus: ProjectStatus) => {
    setTouchDragProject({ id: projectId, sourceStatus });
    setDraggingProjectId(projectId);
  };

  const handleTouchDragEnd = () => {
    setTouchDragProject(null);
    setDraggingProjectId(null);
  };

  const handleTouchDrop = (targetStatus: ProjectStatus) => {
    if (touchDragProject) {
      handleMoveProject(touchDragProject.id, touchDragProject.sourceStatus, targetStatus);
      handleTouchDragEnd();
    }
  };
  ...
  const projectsByStatus = KANBAN_COLUMNS.reduce(
    (acc, col) => {
      acc[col.id] = projects
        ...
        <div className="flex gap-4 h-full">
          {KANBAN_COLUMNS.map(column => (
            <KanbanColumnComponent
              ...
              touchDragProject={touchDragProject}
