...
export function buildLocationDisplayKey(location: { primary: string; secondary?: string }): string {
  const primary = String(location.primary ?? '')
    .trim()
    .toLowerCase();
  const secondary = String(location.secondary ?? '')
    .trim()
    .toLowerCase();
