...
export const EventCard: React.FC<EventCardProps> = ({
  ...
  if (offsetX > 0 || (isExiting && offsetX > 0)) {
    ...
    gradientStyle = {
      background:
        'linear-gradient(to right, rgba(34, 197, 94, 1) 0%, rgba(34, 197, 94, 0.8) 40%, rgba(34, 197, 94, 0) 100%)',
    }
    ...
    if (isAttending) {
      ...
      gradientStyle = {
        background:
          'linear-gradient(to left, rgba(234, 179, 8, 1) 0%, rgba(234, 179, 8, 0.8) 40%, rgba(234, 179, 8, 0) 100%)',
      }
      ActionIcon = LogOut
      actionText = 'Leave'
    } else {
      ...
      gradientStyle = {
        background:
          'linear-gradient(to left, rgba(239, 68, 68, 1) 0%, rgba(239, 68, 68, 0.8) 40%, rgba(239, 68, 68, 0) 100%)',
      }
      ActionIcon = X
      actionText = 'Hide'
    }
  ...
  return (
    <div className={wrapperClass}>
      <div className="overflow-hidden">
        {' '}
        {/* Inner wrapper needed for grid transition */}
        <div className="relative w-full h-full select-none rounded-xl bg-slate-900">
          ...
          >
            ...
            <div
              className={`flex items-center gap-2 font-bold text-white ${offsetX > 0 || (isExiting && offsetX > 0) ? 'opacity-100' : 'opacity-0'}`}
            >
              {ActionIcon && (offsetX > 0 || isExiting) && (
                <ActionIcon className="w-8 h-8 drop-shadow-md" />
              )}
          ...
          <Card
            ...
            style={{
              ...
              transition: isDragging
                ? 'none'
                : `transform ${isExiting ? SLIDE_DURATION : SNAP_DURATION}ms cubic-bezier(0.2, 0.8, 0.2, 1), background-color 0.2s`,
          ...
          >
            <div className="flex justify-between items-start mb-2">
                ...
                {host ? (
                  <>
                    ...
                    <div className="flex flex-col">
                      <div
                        className={`text-xs leading-tight ${isInvolved ? 'text-white/90' : 'text-slate-400'}`}
                      >
                        <span
                          className={`font-medium ${isInvolved ? 'text-white' : 'text-slate-200'}`}
                        >
                          {host.name}
                        </span>
                      </div>
                      <div
                        className={`text-[10px] font-bold uppercase tracking-wider ${badgeColor}`}
                      >
                        {event.activityType}
                      </div>
                    </div>
                  ...
                  <>
                    <div
                      className={`w-8 h-8 rounded-full bg-slate-700 animate-pulse border-2 ${isInvolved ? 'border-white/30' : 'border-slate-600'}`}
                    />
                    <div className="flex flex-col">
                      <div
                        className={`text-xs leading-tight ${isInvolved ? 'text-white/90' : 'text-slate-400'}`}
              ...
              <div className="flex items-center gap-2">
                ...
                {isHost && (
                  <div
                    className={`p-1 rounded-full ${isInvolved ? 'bg-white/20' : 'bg-yellow-500/10'} backdrop-blur-sm`}
                    title="Hosting"
                  >
                ...
                {isAttending && !isHost && (
                  <div
                    className={`p-1 rounded-full ${isInvolved ? 'bg-white/20' : 'bg-green-500/10'} backdrop-blur-sm`}
                    title="Going"
                  >
                    <CheckCircle2
                      className={`w-4 h-4 ${isInvolved ? 'text-white' : 'text-green-500'}`}
                    />
                  </div>
                )}
              </div>
            ...
            <h3
              className={`font-bold mb-1 transition-colors ${isCompact ? 'text-sm' : 'text-lg'} ${textColorPrimary}`}
            >
              {event.title}
            </h3>
            ...
            <div className={`space-y-1.5 text-sm mb-3 ${textColorSecondary}`}>
              <div className="flex items-center space-x-2">
                <Calendar
                  className={`w-4 h-4 ${isInvolved ? 'text-white/70' : 'text-secondary'}`}
                />
                <span>
                  {dateString} •{' '}
                  <span className={event.isFlexibleStart ? 'italic opacity-80' : ''}>
                    {timeString} {event.isFlexibleStart ? '(ish)' : ''}
                  </span>
                </span>
              </div>
              <div className="flex items-center space-x-2">
            ...
            {!isCompact && (
              <div
                className={`flex items-center justify-between mt-3 pt-3 border-t ${isInvolved ? 'border-white/20' : 'border-slate-700/50'}`}
              >
                <div className={`flex space-x-3 text-xs ${textColorSecondary}`}>
                  <div className="flex items-center space-x-1">
                    ...
                    {spotsLeft !== null && (
                      <span
                        className={`ml-1 ${isInvolved ? 'text-white font-bold' : 'text-secondary'}`}
                      >
                        ({spotsLeft} left)
                      </span>
                    )}
                ...
                <div
                  className={`flex items-center space-x-1 text-xs ${isInvolved ? 'text-white/60' : 'text-slate-500'}`}
                >
