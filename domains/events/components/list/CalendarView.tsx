import { CheckCircle2, ChevronLeft, ChevronRight, Clock, Crown } from 'lucide-react';
    ...
    return (
      <div className="flex items-center justify-between p-2 md:p-4 bg-slate-800 border-b border-slate-700 shrink-0 z-10 gap-2">
        <div className="flex items-center gap-2 md:gap-4 overflow-hidden">
          {/* Title */}
          <h2 className="text-base md:text-xl font-bold text-white truncate whitespace-nowrap">
            {title}
          </h2>
          ...
          <div className="hidden md:flex items-center gap-2">
            <div className="flex bg-slate-900 rounded-lg p-0.5 border border-slate-700">
              <button
                type="button"
                onClick={handlePrev}
                className="p-1 md:p-1.5 hover:bg-slate-700 rounded-md text-slate-400 hover:text-white"
              >
            ...
            <button
              type="button"
              onClick={handleToday}
              className="text-xs md:text-sm px-2 md:px-3 py-1.5 rounded-md bg-slate-700 text-slate-300 hover:text-white hover:bg-slate-600 transition-colors"
            >
              Today
            </button>
          </div>
      ...
      <div className="flex flex-col h-full bg-slate-800 select-none">
        ...
        <div className="grid grid-cols-7 border-b border-slate-700 bg-slate-800/80 backdrop-blur shrink-0">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
            <div
              key={day}
              className="py-2 text-center text-[10px] md:text-xs font-bold text-slate-500 uppercase"
            >
              {day}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7 flex-1 auto-rows-fr bg-slate-700 gap-[1px] border-b border-l border-slate-700 overflow-y-auto">
          {/* Padding Days */}
          {[...Array(firstDayOfMonth).keys()].map(n => (
            <div key={`pad-start-${n}`} className="bg-slate-900/30 min-h-[60px] md:min-h-[80px]" />
          ))}
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div
                key={day}
                className="bg-slate-900 p-1 md:p-2 min-h-[60px] md:min-h-[80px] hover:bg-slate-800 transition-colors group relative flex flex-col gap-1"
              >
                <div
                  className={`text-[10px] md:text-xs font-bold w-5 h-5 md:w-6 md:h-6 flex items-center justify-center rounded-full mb-0.5 md:mb-1 ${isToday ? 'bg-primary text-white' : 'text-slate-400'}`}
                >
                  {day}
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`
                           ...
                           truncate cursor-pointer transition-colors w-full text-left ${classes}
                         `}
                      >
                        {icon}
          ...
          {/* End Padding */}
          {[...Array((7 - ((firstDayOfMonth + daysInMonth) % 7)) % 7).keys()].map(n => (
            <div key={`pad-end-${n}`} className="bg-slate-900/30" />
          ))}
        </div>
      </div>
    );
          ...
          {days.map(date => {
            ...
            return (
              <div
                ...
                className={`flex-1 py-2 md:py-3 text-center border-r border-slate-700 last:border-r-0 ${isToday ? 'bg-primary/5' : ''}`}
              >
                <div
                  className={`text-[10px] md:text-xs uppercase font-bold ${isToday ? 'text-primary' : 'text-slate-500'}`}
                >
                  {date.toLocaleDateString('en-US', { weekday: 'short' })}
                </div>
                <div
                  className={`text-sm md:text-lg font-bold ${isToday ? 'text-primary' : 'text-white'}`}
                >
                  {date.getDate()}
                </div>
              </div>
          ...
          <div className="flex relative min-h-[1440px]">
            ...
            {days.map(date => {
              ...
              return (
                <div
                  key={date.toISOString()}
                  className={`flex-1 border-r border-slate-800 relative last:border-r-0 ${isToday ? 'bg-slate-800/20' : ''}`}
                >
                  {/* Hour lines */}
                  {hours.map(h => (
                  ...
                  {dayEvents.map(event => {
                    ...
                    return (
                      <button
                        ...
                        className={`
                             absolute left-0.5 right-0.5 md:left-1 md:right-1 rounded md:rounded-md p-1 md:p-2 cursor-pointer overflow-hidden transition-all text-xs z-10 group text-left w-full
                             ${classes}
                           `}
                        style={{
                          top: `${startMinutes}px`,
                      ...
                      >
                        <div className="flex items-center gap-1 mb-0.5">
                          {icon}
                          <div
                            className={`font-bold truncate text-[10px] md:text-xs ${isInvolved ? 'text-white' : theme.text}`}
                          >
                            {event.title}
                          </div>
                        </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {mode === '3DAY' &&
          renderTimeGrid(Array.from({ length: 3 }, (_, i) => addDays(currentDate, i)))}
