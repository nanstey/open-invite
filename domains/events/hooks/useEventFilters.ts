...
import { getEventSortTime, getEventStartDate, isPastEventSchedule } from '../utils/eventSchedule';
...
function isPastEvent(event: SocialEvent, now: Date) {
  return isPastEventSchedule(event, now);
}
...
function matchesTimeFilter(
  ...
  const eventZero = startOfDay(eventStart);
...
function getGroupTitle(event: SocialEvent, now: Date, statusFilter: StatusFilter) {
  const eDate = getEventStartDate(event);
  if (!eDate) return 'Unscheduled';
...
export function useEventFilters(events: SocialEvent[], currentUserId: string) {
  ...
  const groupedEvents = React.useMemo<EventsGroup[]>(() => {
    ...
    filteredEvents.forEach(event => {
      const title = getGroupTitle(event, now, statusFilter);
